...
void ActiveStress::recompute_tension(const double dt,
                                     const Vector<double> &fiber_stretch) {
  for (unsigned int i = 0; i < states.ncols(); ++i) {
    if (!has_previous_fiber_stretch[i]) {
      previous_fiber_stretch[i] = fiber_stretch[i];
    ...
    refresh_state_for_current_stretch_local(
        fiber_stretch[i], fiber_stretch_rate_local, state_loc);
    states.set_col(i, state_loc);

    const double Ta = compute_active_tension_local(state_loc);
    raw_active_tension[i] = Ta;
    const double Ka = compute_active_stiffness_local(
        state_loc, fiber_stretch[i], fiber_stretch_rate_local);
    active_stiffness[i] = Ka;

    if (use_stabilization) {
      active_tension[i] = apply_stabilization_local(
          Ta, Ka, fiber_stretch[i], previous_fiber_stretch[i]);
    } else {
      active_tension[i] = Ta;
    }
  }
