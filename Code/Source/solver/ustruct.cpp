    ...
    for (int g = 0; g < fs[0].nG; g++) {
      ...
      if (nsd == 3) {
        ...
        ustruct_3d_m(com_mod, cep_mod, vmsStab, fs[0].eNoN, fs[1].eNoN, nFn, w,
                     ...
                     ya_l_n, ka_l_f, ka_l_s, ka_l_n, lambda_prev_l,
                     lR, lK, lKd);
...
void ustruct_2d_m(ComMod &com_mod, CepMod &cep_mod, const bool vmsFlag,
  ...
  for (int a = 0; a < eNoNw; a++) {
    ...
    ka_g_f = ka_g_f + Nw(a) * ka_l_f(a);
    ka_g_s = ka_g_s + Nw(a) * ka_l_s(a);
    ka_g_n = ka_g_n + Nw(a) * ka_l_n(a);
    lambda_prev_g = lambda_prev_g + Nw(a) * (lambda_prev_l(a) - 1.0);
  }
  ...
  // isochoric elasticity tensor in Voigt notation (Dm). The Regazzoni
  // stabilization correction (ka_g * (lambda_current - lambda_prev_g)) is
  // applied inside compute_pk2cc, using the current fiber stretch derived
  ...
  mat_models::compute_pk2cc(com_mod, cep_mod, eq.dmn[cDmn], F, nFn, fN, ya_g_f,
                            ya_g_s, ya_g_n, ka_g_f, ka_g_s, ka_g_n,
                            lambda_prev_g, Siso, Dm, Ja);
