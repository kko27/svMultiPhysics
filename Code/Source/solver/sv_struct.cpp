...
void construct_dsolid(ComMod& com_mod, CepMod& cep_mod, const mshType& lM, const SolutionStates& solutions)
{
  ...
  Vector<double> pSl(nsymd), ya_l_f(eNoN), ya_l_s(eNoN), ya_l_n(eNoN),
      ka_l_f(eNoN), ka_l_s(eNoN), ka_l_n(eNoN), lambda_prev_l(eNoN), N(eNoN);
  ...
  for (int e = 0; e < lM.nEl; e++) {
    ...
    for (int a = 0; a < eNoN; a++) {
      ...
      if (eq.dmn[cDmn].active_stress != nullptr) {
        ...
        lambda_prev_l(a) = cep_mod.cem.lambda_prev[Ac];
      }
    }
...
void struct_2d(ComMod &com_mod, CepMod &cep_mod, const int eNoN, const int nFn,
               ...
               const Vector<double> &ka_l_n, const Vector<double> &lambda_prev_l,
               Array<double> &lR, Array3<double> &lK) {
  using namespace consts;
  ...
  for (int a = 0; a < eNoN; a++) {
    ...
    lambda_prev_g = lambda_prev_g + N(a) * (lambda_prev_l(a) - 1.0);
  }
  #ifdef debug_struct_2d
  dmsg << "ud: " << ud(0) << " " << ud(1);
  dmsg << "F: " << F(0,0);
  ...
  // The Regazzoni stabilization correction (ka_g * (lambda_current - lambda_prev_g))
  // is applied inside compute_pk2cc, using the current fiber stretch derived
  // from F itself so it stays consistent with the tangent contribution.
  Array<double> S(2,2), Dm(3,3);
  double Ja;
  mat_models::compute_pk2cc(com_mod, cep_mod, dmn, F, nFn, fN, ya_g_f, ya_g_s,
                            ya_g_n, ka_g_f, ka_g_s, ka_g_n, lambda_prev_g, S,
                            Dm, Ja);
...
void struct_3d(ComMod &com_mod, CepMod &cep_mod, const int eNoN, const int nFn,
  ...
  // Voigt notationa (Dm). The Regazzoni stabilization correction
  // (ka_g * (lambda_current - lambda_prev_g)) is applied inside
  // compute_pk2cc, using the current fiber stretch derived from F itself
  // so it stays consistent with the tangent contribution.
  //
